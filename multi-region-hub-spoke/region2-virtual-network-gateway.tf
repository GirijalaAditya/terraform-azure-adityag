...
resource "azurerm_subnet" "subnet_gateway_region2" {
  name                                          = "GatewaySubnet"
  address_prefixes                              = [cidrsubnet(var.vnet_hub_region2_address_space, 6, 3)]
...
resource "azurerm_virtual_network_gateway" "vng_region2" {
  ...
  ip_configuration {
    name                          = "ipconfig-vng"
    public_ip_address_id          = module.azure_vng_public_ip_region2.public_ip_address_id
  ...
  vpn_client_configuration {
    address_space        = var.vpn_region2_client_configuration.vpn_address_space
    aad_audience         = var.vpn_region2_client_configuration.aad_audience
