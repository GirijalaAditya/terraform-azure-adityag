resource "azurerm_point_to_site_vpn_gateway" "p2s_vpn_gateway_vhub_region2" {
  name                        = "p2s-vpn-gateway-${var.region2}"
  ...
  connection_configuration {
    ...
    vpn_client_address_pool {
      address_prefixes = [var.vpn_client_address_pool_region2]
    }
  }
}